    ...
    for(i=0; i < tst; ++i){
        ...
        for(j = 0; j < tsc; ++j){
            if(sc[j] == st[i+j] && sc[j]=='1' && tsc == 1 && st[i+j+1] != '0'){flag = 1; break;}
                ...
                if(sc[j] == st[i+j]){flag = 1;} else {flag = 0; break;}
            }
        }
    }
    return flag;
...
int check(void ** tab){
    ...
    for(i=1; i <= NumberOfNodes && flag == 1; ++i){
        for(j = 0; j<NumberOfNodes && flag == 0; ++j){
            tmp = tab[i];
            while(tmp != NULL && flag == 0){
                flag = test_sous_chaine(intToString(i+1), intToString(tmp->value));
                tmp = tmp->l;
            }
        }
    }
    if(flag == 0){printf("le sommet %i n'est pas représenté\n", i); return i;}
    return 0;
}
...
void afficheTab(void ** tab){
    ...
    for(i=0; i < NumberOfNodes; ++i){
        if(tab[i] != NULL){afficheListe(tab[i]);printf("\n");}
    }
}

int concateneEntier(int a1, int a2, int a3){ return ((a1*(10*calculTailleEntier(a2))+a2)*10*(calculTailleEntier(a3)))+a3; }
